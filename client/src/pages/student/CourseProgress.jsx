    ...
    if (response?.success) {
      if (!response?.data?.isCoursePurchased) {
        ...
        if (response?.data?.progress?.length === 0) {
          ...
          const lastViewedLectureIndex = response?.data?.progress.reduceRight(
            (acc, currObj, index) => {
              return acc === -1 && currObj.viewed ? index : acc;
            },
            -1
          );
          setCurrentLecture(
            response?.data?.courseDetails?.curriculum[
              lastViewedLectureIndex + 1
            ]
          );
        }
      }
    ...
    <div className="flex flex-col h-screen bg-[#1c1d1f] text-white">
      ...
      <Dialog open={lockCourse}>
        <DialogContent className="sm:w-[450px]">
          <DialogHeader>
            <DialogTitle>You are not allowed to access this Page</DialogTitle>
            <DialogDescription>
        ...
        <DialogContent showOverlay={false} className="sm:w-[450px]">
          <DialogHeader>
            ...
            <DialogDescription className="flex flex-col gap-3">
              <Label>You have successfully completed this course</Label>
              <div className="flex flex-row gap-3">
                <Button onClick={() => navigate("/student-courses")}>
                  My Courses
                </Button>
                <Button onClick={revisitCourse}>Revisit Course</Button>
              </div>
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
